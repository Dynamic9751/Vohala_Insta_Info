...
def printlogo():
    ...
    pc.printout("\nVersion 1.1 - Developed by Vohala *Daryl Daruwala.*\n\n", pc.YELLOW)
...
def cmdlist():
    ...
    pc.printout("fwingsemail\t",pc.RED)
    print("Get email of users followed by target * Don't use this * ")
    pc.printout("fwersnumber\t")
    print("Get phone number of target followers ")
    pc.printout("fwingsnumber\t",pc.RED)
    print("Get phone number of users followed by target * Don't use this * ")    
...
parser = argparse.ArgumentParser(description='Vohala is a OSINT tool on Instagram. It offers an interactive shell '
...
api = Vohala(args.id, args.file, args.json, args.command, args.output, args.cookies)
